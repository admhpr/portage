...
let dist_between_max = 66; // lower to make less connection, checked and recalibrated every resize
// span of connections
const CON_RADIUS = 100;
...
function Star() {
    ...
    this.update = () => {
        ...
        this.x = Math.cos(ROTATION) * (this.x - centerCanvas.x) - Math.sin(ROTATION) * (this.y - centerCanvas.y) + centerCanvas.x
        this.y = Math.sin(ROTATION) * (this.x - centerCanvas.x) + Math.cos(ROTATION) * (this.y - centerCanvas.y) + centerCanvas.y
    }
...
function connectStars() {
    for (let i = 0, star1; star1 = stars[i]; i++) {
        for (let j = i + 1, star2; star2 = stars[j]; j++) {
            ...
            if (xDiff < dist_between_max && xDiff > -dist_between_max &&
                yDiff < dist_between_max && yDiff > -dist_between_max &&
                xCoreDiff < CON_RADIUS && xCoreDiff > -CON_RADIUS &&
                yCoreDiff < CON_RADIUS && yCoreDiff > -CON_RADIUS) {
                ctx.beginPath()
